...
from config import PHASE, DEFAULT_LANG, lang_directive
...
def _plan(state, profile, lang=DEFAULT_LANG) -> dict:
    """② 军师：产出作战计划(纯决策)。"""
    user = (
        f"客户状态：\n{state.to_json()}\n\n"
        f"当前节点：{profile.name}\n目标：{profile.goal}\n"
    ...
    plan = json.loads(chat(PLAN_SYSTEM + lang_directive(lang), user, json_mode=True, mock_hint="plan"))
...
def _generate(state, plan, tool_results, lang=DEFAULT_LANG) -> str:
    ...
    return chat(GEN_SYSTEM + lang_directive(lang), user, mock_hint="generate")


def regenerate(state, plan, tool_results, prev_message, instruction, lang=DEFAULT_LANG) -> str:
...
def run_engine(state, conversation, behaviors, profile, perceived=None, lang=DEFAULT_LANG) -> dict:
    """跑引擎，返回结构化结果(不打印)。
    perceived 传入时跳过①感知(用于无 key 的占位演示)。
    lang: 目标语言/市场，注入②策略③④话术的 prompt，让输出用该语言与当地商务风格。"""
    # ① 感知 + 客户分级
    if perceived is None:
        perceived = perceive(conversation, behaviors, lang)
    state.hwc = perceived.get("hwc", state.hwc)
    state.current_node = perceived.get("node", state.current_node)

    # ② 策略(军师)：只决策
    plan = _plan(state, profile, lang)

    # ③ 执行·调用子agent/工具（入参=state，工具自取，不信任 LLM 编的参数）
    tool_results = _execute(state, plan)

    # ④ 执行·生成话术
    msg = _generate(state, plan, tool_results, lang)
